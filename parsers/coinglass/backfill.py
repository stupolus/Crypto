...
def backfill_providers(
    ...
    *,
...
) -> tuple[StaticLiquidationProvider, StaticOpenInterestProvider]:
    """Скачать историю и собрать Static-провайдеры под backtest.

    Возвращает (liq_provider, oi_provider). Пустые если symbol не в
    маппинге ИЛИ план Coinglass не активен (клиент → []).
    """
    ...
    if mapping is None:
        ...
        return StaticLiquidationProvider(), StaticOpenInterestProvider()
    ...
    logger.info(
        "backfill %s: %d liq-buckets, %d oi-points (%s)",
        bingx_symbol,
        len(buckets),
        len(oi_series),
