"""Строгая валидация smc_liqsweep (план 32.2).

15m мажоры (есть данные; DOGE — мемкоин-прокси с реальной
глубиной/ликвидностью). Гейт СТРОГИЙ (тот же, что план 31):
OOS PF>1.5 И Sharpe>1.0 И ≥100 сделок, IS≈OOS, walk-forward,
overlap-коррекция, cost-sweep 0.10/0.15/0.20%. Любой провал,
особенно cost-sweep → отклонить. Скептический априор: SMC в
массе — догма, edge маловероятен (план 32).
"""
...
def main() -> None:
    ...
    for sym in _COINS:
        ...
        subprocess.run(
            [sys.executable, "-m", "scripts.run_backtest", "--strategy",
             "smc_liqsweep", "--symbol", sym, "--candles", candle,
             "--split-fraction", "0.5"],
    ...
    if not raw:
        print("нет сделок (smc_liqsweep не сработал на 15m мажорах)")
        return
    raw.sort()
    split = raw[len(raw) // 2][0]
    print("smc_liqsweep — строгая валидация (15m мажоры, план 32.2)")
    ...
    print(
        "ВЕРДИКТ: "
        + (
            "прошёл строгий гейт+cost-sweep — кандидат (план 32.3/29)"
            if overall_pass
            else "НЕ прошёл (cost-sweep/гейт) — отклонить, как и ожидалось"
        )
    )
    print("SMC в массе — недоказанная догма; свип-reclaim — единственный")
    print("фальсифицируемый кусок. Не прошло на ликвиде → отклонить.")
