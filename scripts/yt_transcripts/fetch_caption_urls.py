...
def pick_track(info):
    ...
    # fallback: any *-orig, then anything
    for src_name, src in (("subtitles", subs), ("automatic", auto)):
        keys = sorted(src, key=lambda k: (not k.endswith("-orig"), k))
        for lang in keys:
            for f in src[lang]:
                if f.get("ext") == "json3":
                    return lang, src_name, f["url"]
    ...
    for v in videos:
        ...
        for attempt in (1, 2):
            try:
                ...
                if url:
                    rec.update(lang=lang, caption_source=src,
                               url=url, status="ok")
                else:
                ...
                if attempt == 1:
                    time.sleep(5)
        manifest.append(rec)
        json.dump(manifest, open(out_path, "w"),
                  ensure_ascii=False, indent=1)
        print(f"[{v['n']:>3}/{len(videos)}] {v['id']} -> "
              f"{rec.get('status')} {rec.get('lang','')}", flush=True)
        time.sleep(1)
