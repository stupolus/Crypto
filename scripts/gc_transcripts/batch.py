...
SKIP_MARK = ("Есть задание", "тестирование", "Учебный план",
             "Список литературы")
...
def ensure_model():
    ...
    for f in ("config.json", "tokenizer.json", "vocabulary.txt",
              "model.bin"):
        subprocess.run(
            ["curl", "-sL", "--fail", "--retry", "3", "--max-time", "1200",
             "-o", str(MODEL / f),
             f"https://huggingface.co/Systran/faster-whisper-small/"
             f"resolve/main/{f}"],
            check=True,
        )
...
def fetch_audio(lid):
    ...
    if txt.exists() and txt.stat().st_size > 200:
        return None  # already transcribed earlier, audio not needed
    ...
    with sync_playwright() as p:
        ...
        ctx = b.new_context(storage_state=str(STATE),
                             ignore_https_errors=True, locale="ru-RU",
                             user_agent=UA)
        pg = ctx.new_page()
        pg.on("request", lambda r: got.__setitem__("u", r.url)
              if "/api/playlist/master/" in r.url and not got["u"] else None)
        pg.goto(f"{BASE}/teach/control/lesson/view/id/{lid}",
        ...
        media = next((ln.strip() for ln in master.splitlines()
                      if ln.strip().startswith("http")
                      and "/media/" in ln and "/360?" in ln), None)
        if not media:
            ctx.close()
            b.close()
            return "NO_VIDEO"
        segs = [ln.strip() for ln in pg.request.get(media).text().splitlines()
                if ln.strip().startswith("http")]
        ...
        b.close()
    subprocess.run(
        ["ffmpeg", "-y", "-loglevel", "error", "-i", str(ts), "-vn",
         "-ac", "1", "-ar", "16000", "-c:a", "libmp3lame", "-q:a", "5",
         str(mp3)], check=True, timeout=900)
...
def transcribe(lid, mp3):
    ...
    if _MODEL is None:
        ...
        _MODEL = WhisperModel(str(MODEL), device="cpu",
                              compute_type="int8", cpu_threads=4)
    segments, _ = _MODEL.transcribe(
        mp3, language="ru", beam_size=1, vad_filter=True,
...
def commit_push(path, n, title):
    git("add", str(path))
    git("-c", "core.pager=cat", "commit", "-q", "-m",
...
def main():
    ...
    vids = [x for x in lessons
            if not any(m in x["t"] for m in SKIP_MARK)]
    ...
    for x in vids:
        ...
        try:
            ...
            (WORK / "batch_failures.log").open("a").write(
                f"{n} {lid} {e}\n")
            continue
