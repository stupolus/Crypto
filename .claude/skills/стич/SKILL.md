...
name: стич
description: >-
  Stitch (Google) дизайн-скил. Вызывается командой /стич. Генерация новых
  экранов из текста или изображений, редактирование существующих экранов
  промптами и токенами дизайн-системы, генерация вариантов — через Stitch MCP.
  Включает конвейер улучшения промптов, маппинги дизайна, профессиональную
  UI/UX-терминологию, дизайн-токены и тему. Generate new screens from text
  prompts or images, edit existing screens with design system tokens, and
  generate design variants using Stitch MCP.
allowed-tools:
  - "stitch*:*"
  - "Bash"
  - "Read"
...
# Стич (Stitch · Generate Design)

> Это скил из репозитория [google-labs-code/stitch-skills](https://github.com/google-labs-code/stitch-skills)
> (`plugins/stitch-design/skills/generate-design`), установленный локально под
> командой **`/стич`**. Содержимое воспроизведено дословно; добавлены только
> вводная заметка и требования ниже.

> [!IMPORTANT]
> **Требуется Stitch MCP-сервер.** Все инструменты вида `list_projects`,
> `generate_screen_from_text`, `edit_screens`, `generate_variants` приходят из
> Stitch MCP. Если сервер не подключён в окружении агента — сначала настройте
> его, иначе скил не сможет вызывать инструменты Stitch.
>
> Связанные скилы того же репозитория (`manage-design-system`,
> `upload-to-stitch`, `code-to-design` и др.) здесь не установлены — при
> необходимости их можно добавить отдельно из репозитория. Ниже на них есть
> ссылки как на делегируемые шаги.
