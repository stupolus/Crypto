...
⚠️ EDGE НЕ ПОДТВЕРЖДЁН (план 23 закрыт 2026-05-15, аудит 2026-05-27).
    - 10 DOLF-детекторов × BTC/ETH/SOL/ADA/HYPE на Coinglass HOBBYIST
      (1d, ~2.5г) → 9 мимо первичного гейта.
    - l3 (OI drop + flat price) прошёл первичный, провалил kill-тесты:
      ETH PF 0.74 (coin-consistency), портфель overlap-корр. p=0.794
      (неотличим от нуля), BTC-only p≈0.05 = selection bias.
    - Та же смерть, что у LTC/funding/trailing — паттерн «выживает
      первичный гейт, умирает под честными kill-тестами».

**Live-запуск заблокирован** в `runners/live_runner.py` (raise при
попытке без `LIQ_REVERSAL_FORCE_LIVE=1`). Стратегия сохранена как:
- структурный реестр сетапов (A1 LONG, A2 SHORT — DOLF-композиты);
- образец composite-стратегии на DI-провайдерах;
- сравнительный baseline для будущих DOLF-исследований на других
  данных (платная глубокая история / иной таймфрейм).

См. `retro/2026-05-15-поиск-edge-итог.md` (исходный закрывающий ретро)
и `retro/2026-05-27-щукин-аудит.md` (текущий аудит).
