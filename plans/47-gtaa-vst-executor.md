...
Решение владельца (2026-05-22): **(A)** принять, что нефть
может не входить из-за liq-буфера при широком стопе Faber
(корректное поведение RiskEngine, не баг) — аллокация идёт по
проходящим риск-проверку активам. **+ добавить серебро**
(`SI=F` → `NCCOXAG2USD`) пятым активом. Универс становится
5 активов, equal-weight `1/5`.

> ⚠️ **Честная оговорка по серебру.** Академический гейт плана 43
> (OOS Sharpe 1.09) был на 5 активах **с облигациями**, side-test
> плана 45 — на 4 (без bond-перпа на BingX). Серебро — НЕ
> bond-leg академического универса; его самостоятельный edge **не
> бэктестился**. Для demo (цель — корректность ИСПОЛНЕНИЯ, не PnL)
> это допустимо: серебро расширяет покрытие исполнения ещё на один
> commodity-перп. Но **до LIVE** silver-leg требует отдельной
> edge-проверки (бэктест/WF) по правилам проекта (CLAUDE.md п.1/6,
> AGENTS.md). Пока — только VST-исполнение.
...
| `SI=F` | `NCCOXAG2USD-USDT` (добавлен 2026-05-22) |

Все в hedge-mode (фикс #164 разблокировал rebalance).

## Правило (канон Faber 2007, FIXED, плана 43)

На последний торговый день месяца (по Yahoo-данным):
1. Для каждого из индексов универса: signal = `close > SMA200`.
2. Целевая аллокация: equal-weight `1/5` эквити **per ON-актив**,
...
3. Реконсиляция к target позиции через RiskEngine на каждый
   ON-актив (1/5 эквити риск-сайз 1% B-tier).
...
- Скрипт track'ит `state.last_rebalance_eom` (дата). Если
  максимальная Yahoo-EOM-дата по всем активам > last_rebalance_eom
...
- `_signals_from_eoms`: 5 активов, all ON → 1/5 каждый;
  частичные OFF → cash; all OFF → flat портфель.
- `test_universe_includes_silver`: универс = 5 активов,
  серебро (SI=F → NCCOXAG2USD) присутствует.
...
4. EOM-обнаружение: разные индексы могут иметь разные
   trading calendars (^NDX vs GC=F vs CL=F vs SI=F). Беру **max
   EOM** по всем активам → консервативно (ребаланс только когда
   ВСЕ индексы обновились).
5. RiskEngine может отклонить часть из 5 активов (например
   нефть при широком стопе/liq-буфере) → partial allocation,
   не 1/5. Нормально, лог reject (решение владельца A, 2026-05-22).
...
- DEMO_CRITERIA 2/3: SMA200 по 5 индексам на реальных Yahoo-данных
...
2. **SMA200 корректна.** По каждому из 5 индексов SMA200 считается
...
4. **Аллокация ⅕.** Каждый ON-актив получает ≈1/5 эквити (с
   поправкой на RiskEngine reject/clamp); OFF — `target_qty=0`.
   Поле `equity_share`, `target_qty`. Отклонения из-за liq-буфера
   (напр. нефть при широком стопе) — ожидаемы, см. решение A.
...
- Канон FIXED: SMA=200, monthly EOM, equal-weight 1/5 ON,
  5 активов выше. БЕЗ оптимизации. Серебро — execution-only
  до отдельного бэктеста edge (см. оговорку выше).
