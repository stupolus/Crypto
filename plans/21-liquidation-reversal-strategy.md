...
## ⚠️ AUDIT-DEPRECATED 2026-05-27

Аудит источника (Щукин) + реализации провален — см.
`retro/2026-05-27-аудит-щукина.md`. Кратко:
- Никогда не валидирована WF (фаза 21.4 не выполнена; Coinglass-тариф
  недостаточен — план 32).
- Параметры «не подгоняем», но `бизнес/материалы/курсы/dmitry-shukin/`
  фактически **пуст** — нет документальной базы.
- Жёсткий gate «не шортить пока OI растёт» — догма ⚠️ по собственной
  классификации channel doc.
- Косвенный негатив: composite_signal v2 (та же Щукин-композиция) дал
  OOS PF 0.49 / −4.2% BTC 4h.

⇒ `liquidation_reversal` удалена из `--strategy` choices раннеров
(live + backtest). Код+тесты остаются как историческое; возврат —
только после валидных данных + повторного WF-аудита (отдельный PR).
NO-GO держится.
